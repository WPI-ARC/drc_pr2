...
class TrajectoryPlanner:
            ...
            if(appr_res == 1):
                ...
                if(self.generate_points() == 1):
                    ...
                    if(self.arm_planner() == 1):
                        ...
                        res.success_code = "arm planner succeeded"
                    else:
    ...
    def generate_points(self):
        ...
        # InSyncWithROS sets DOF values for both OpenRAVE and Gazebo
        #self.SetRobotDOFValuesInSyncWithROS(self.robot,[-0.85,-0.1,-1.00,-1.9,0.0,-1.00,-1.00,0.055],[27,28,29,30,31,32,33,34])

        #self.SetRobotDOFValuesInSyncWithROS(self.robot,[0.85,-0.1,1.00,-1.9,0.0,-1.00,1.00,0.055],[15,16,17,18,19,20,21,22])
        ...
        self.T0_LH1_ENTRY = dot(temp,MakeTransform(matrix(rodrigues([0,0,0])),transpose(matrix([0,0.0,-0.15]))))
        ...
        self.T0_RH1_ENTRY = dot(temp,MakeTransform(matrix(rodrigues([0,0,0])),transpose(matrix([0,0.0,-0.15]))))
    ...
    def arm_planner(self):
        ...
        time.sleep(0.5)
        try:
            #answer = self.probs[0].SendCommand('RunCBiRRT psample 0.2 jointgoals %d %s %s'%(len(goaljoints),Serialize1DMatrix(matrix(goaljoints)),TSRChainString_A1))
