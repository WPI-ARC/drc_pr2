...
def run():
    ...
    env.Load('../../models/crank.robot.xml')
    robotid = env.ReadRobotURI('robots/pr2-beta-static.zae')
    crankid = env.GetRobots()[0]

    # Move the crank to the front of the robot, somewhere it can grasp and rotate
    crankid.SetTransform(array(MakeTransform(matrix(rodrigues([0,pi/2,0])), transpose(matrix([0.55,0,0.8])))))
    ...
    initconfig_candidate_0 = robotid.GetActiveDOFValues()
    ...
    # Note: GetDOFValues() returns all 39 joints' values, whereas GetActiveDOFValues returns only the active joints' values
    initconfig_candidate_2 = robotid.GetActiveDOFValues()
    ...
    T0_w0L = eye(4)
    T0_w0R = MakeTransform(rodrigues([0,0,0]),matrix([[jointtm[9]],[jointtm[10]],[jointtm[11]]]))
    ...
    #y: 0.0842
    #T0_LH2 = MakeTransform(matrix(dot(rodrigues([-crank_rot,0,0]),dot(rodrigues([pi/2,0,0]),rodrigues([0,0,pi/2])))),transpose(matrix([0.51,0.0842,0.7293]))) # --> calculated by hand
    #T0_RH2 = MakeTransform(matrix(dot(rodrigues([-crank_rot,0,0]),dot(rodrigues([-pi/2,0,0]),rodrigues([0,0,-pi/2])))),transpose(matrix([0.51,-0.0842,0.8707]))) # --> calculated by hand
    ...
    robotid.SetActiveDOFValues(initconfig_candidate_0)
    ...
    #T0_w0L = MakeTransform(matrix(rodrigues([0,0,0])),transpose(matrix([0,0,0])))
    T0_w0L = eye(4)
