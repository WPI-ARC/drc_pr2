...
if(robottype == 1)    
    orBodySetTransform(crankid,[reshape(rodrigues([0 pi/2 0]),1,9)';[0.35 0.1 0.9]'])
elseif(robottype == 2)    
...
Tw0_eH = eye(4);
Bw0H = [-0.1,0.1,-0.05,0.05,-0.2,0,0,0,-pi,pi,0,0];
TSRString4 = SerializeTSR(4,'NULL',T0_w0H,Tw0_eH,Bw0H);

TSRChainStringGrasping = [SerializeTSRChain(0,1,0,1,TSRString1,'NULL',[]) ' ' SerializeTSRChain(0,1,0,1,TSRString2,'NULL',[]) ' '  SerializeTSRChain(0,1,0,1,TSRString3,'NULLL',[]) ' ' SerializeTSRChain(0,1,0,1,TSRString4,'NULL',[])];
