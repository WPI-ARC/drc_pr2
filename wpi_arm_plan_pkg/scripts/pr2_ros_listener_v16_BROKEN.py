...
class TrajectoryPlanner:
    def __init__(self,args=None):
        path = subprocess.check_output("rosstack find drc", shell=True)
        path = path.strip("\n")
        ...
        self.env.Load(path + '/wpi_openrave/models/driving_wheel.robot2.xml')
        ...
        if(self.save_log):
            ...
            self.my_logger = BensLogger("tepra_data")
            # 0: num_of_iter
            # 1: cbirrt_result
            # 2: gui_id
            # 3: vposx
            ...
            # 11: time
            # 12: notes
            self.my_logger.header(["num_of_iter",
                                   "cbirrt_result",
                                   "gui_id",
                                   "valve_position_x",
                                   "valve_position_y",
        ...
        if(self.mode == "openrave_with_ros"):
            #self.env.SetViewer('qtcoin') # note that if uncommented this will make the service segfault.
            ...
            rospy.Service("openrave_pr2_look_ahead",Empty,self.handle_look_ahead_srv)

            #self.look_ahead() # Reset the head
            ...
            print "*** Loaded OPENRAVE_WITH_ROS planner ***"
    ...
    def handle_look_ahead_srv(self,req):
        self.look_ahead()
        return []        
